        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前路由跳转是从A路由跳转到B路由，此处在加入购物车进行路由跳转之前
                ，先要把你购买的产品的信息通过请求的形式通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  methods: {
    ...
    async addShopCart() {
      try {
        // 派发action，发送请求，将产品加入到数据库（通知服务器）
        /* 当前这里是派发一个action，也向服务器发送请求，判断加入购物车的操作是成功还是失败，进行相应的操作
        this.$store.dispatch('getUpdateShopCart',{skuid:this.$routr.params.skuid,skuNum:this.skuNum}) 
        上面这行代码说白了就是：调用仓库中的getUpdateShopCart函数，这个方法加上async，返回一定是一个Promise对象
        写上return，结果要么成功，要么失败
        */
        ...
        // 服务器存储成功-----进行路由跳转并传递参数
        // 在路由跳转时要将产品信息带过去
        // 简单信息比如skuNum，可以通过query方式直接给路由组件传递过去
        // 产品信息数据【比较复杂：skuInfo】，通过会话存储传递过去。
